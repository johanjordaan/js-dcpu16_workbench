...
var CPU = function() {
  this.on_memory_change_handlers = [];
  this.reset();
}
...
CPU.prototype.mul = function() {
  ...
  this.a.set(mul&0xffff);
  this.overflow = ((mul>>16)&0xffff);
}
...
CPU.prototype.set_memory = function(pos,val) {
  ...
  // Keep track of the last couple of memory changes... This should be outsourced to the 
  // new event system
  //
  if(this.memory_changes.unshift(pos)>this.memory_change_depth)
    this.memory_changes.pop();
    
  // Fire any memory event handlers
  //
  for(var i=0;i<this.on_memory_change_handlers.length;i++){
    var handler = this.on_memory_change_handlers[i];
    if(pos >= handler.begin && pos<= handler.end)
      handler.func(pos);
  }
}
...
CPU.prototype.on_memory_change = function(begin,end,func) {
  this.on_memory_change_handlers.push({begin:begin,end:end,func:func});
}
