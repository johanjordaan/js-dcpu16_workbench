  ...
  for (var i =0;i<label_keys.length;i++) {
    ...
    if(typeof(refs) != 'undefined'){
      for(var j=0;j<refs.length;j++)
        ret_val[refs[j].mem_pos] = label_keys[i];
    } else {
      // This is most likely a ref to an undefined label
    }
  }  
...
deassemble = function(byte_code,labels) {
  ...
  for(var i=0;i<byte_code.length;i++) {
    ...
    if(op==0x0) {
      op_txt = sop_lookup[a];
      if(typeof(op_txt) == 'undefined') {
        var val_txt = '0x'+byte_code[i].toString(16);
        line.txt += 'DAT '+val_txt;
      } else {
