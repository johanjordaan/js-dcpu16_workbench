...
convert_value = function(value_txt) {
  ...
  if(typeof(value) == 'undefined') {
    if(value_txt.indexOf('[')==0) {             // Mem ref with ...
    if(value_txt.indexOf('+')!=-1) {            // ... delta offset
      var tokens = value_txt.replace(/\[|\]/g,'').split('+');       // Replace the [] characters...
      var token_0 = tokens[0].trim();
      var token_1 = tokens[1].trim();

      value = d_nw_value_lookup[token_0];
      if(typeof(value) != 'undefined'){
        value_nw = parseInt(token_1);
        if(isNaN(value_nw)) {
          label_ref = token_1;
          value_nw = 0x0;
        }
      } else {
        value = d_nw_value_lookup[token_1];
        value_nw = parseInt(token_0);
        if(isNaN(value_nw)) {
          label_ref = token_0;
          value_nw = 0x0;
        }
      ...
      if(isNaN(value_nw)) {
        value = 0x1f;
        if(value_txt.indexOf('\'') != -1 || value_txt.indexOf('\"') != -1) {    // Then it is a character literal
          value_txt = value_txt.trim().replace(/\'|\"/g,'');
          value_nw = value_txt[0].charCodeAt(0); 
        } else {
          value_nw = 0x0;
          label_ref = value_txt;
        }
...
handle_label_ref = function(label_refs,label,mem_pos,source_line) {
  if(typeof(label) != 'undefined') {
    if(typeof(label_refs[label]) == 'undefined')
      label_refs[label] = [];
    label_refs[label].push({mem_pos:mem_pos,source_line:source_line});
  }
  return label_refs;
}
...
assemble = function(source) {
  ...
  for(var i=0;i<source_lines.length;i++) {
    var line = source_lines[i].replace(/\t/g,' ').trim();
    if(line == '') continue;
    ...
    if(op_txt == 'DAT') {
      if(parms.length<1) {
        errors.push({line:(i+1),type:'ERROR',msg:'[DAT] Requires at least one parameter.'});
        continue;
      }
      ...
      for(var p=0;p<parms.length;p++) {
        ...
        if(isNaN(data)) {
          data = parms[p].trim()
          if(data.indexOf("\"")==-1) {    // Label ref
            label_refs = handle_label_ref(label_refs,data,byte_code.length,i);
            byte_code.push(0);
          } else {
            data = data.replace(/"/g,'');  
      ...
      if(typeof(op) == 'undefined') {
        ...
        if(typeof(sop) == 'undefined') {
          errors.push({line:(i+1),type:'ERROR',msg:'['+op_txt+'] Undefined op code.'});
          continue;
        }
        if(parms.length<1) {
          errors.push({line:(i+1),type:'ERROR',msg:'['+op_txt+'] Requires at least one parameter.'});
          continue;
        }
        if(parms.length>1)
          errors.push({line:(i+1),type:'WARNING',msg:'['+op_txt+'] Requires only one parameter. Extra parameter/s ignored.'});
        ...
        if(parms.length<2) {
          errors.push({line:(i+1),type:'ERROR',msg:'['+op_txt+'] Requires at least two parameter.'});
          continue;
        }
        if(parms.length>2)
          errors.push({line:(i+1),type:'WARNING',msg:'['+op_txt+'] Requires only two parameter. Extra parameter/s ignored.'});
      ...
      label_refs = handle_label_ref(label_refs,a.label_ref,byte_code.length,i);
      if(typeof(a.nw) != 'undefined') byte_code.push(parseInt(a.nw));
    
      label_refs = handle_label_ref(label_refs,b.label_ref,byte_code.length,i);
  ...
  for(var i=0;i<label_ref_keys.length;i++) {
    ...
    if(typeof(labels[label]) == 'undefined') {
      for(var j=0;j<label_refs[label].length;j++) {
        ...
        errors.push({line:(source_line+1),type:'ERROR',msg:'['+label+'] Referenced but not defined.'});
      }
    }
  }
