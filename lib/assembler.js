...
generate = function(tokens) {
  ...
  while(!done)
  {
    ...
    if(op_index>0 && tokens[op_index-1].type=='LABEL') {
      var new_label = tokens[op_index-1].value;
      labels[new_label] = byte_code.length;
      
      for(var l=0;l<unresolved_labels.length;l++) {
        var unresolved_label = unresolved_labels[l];
        if(unresolved_label.name == new_label) { 
          byte_code[unresolved_label.address] = labels[new_label];
        }
      }
    }
    ...
    for(var i=0;i<parms.length;i++)
    {
      ...
      switch(p.sub_type) {
        ...
        case 'REGISTER_OFFSET':
          ...
          current_instruction[0] = current_instruction[0] | val;
          current_instruction.push(to_int(p.value[0]));
          break;
        case 'NUMBER':
          if(p.value<=0x1f) {
            var val = (to_int(p.value)+0x20)<<shift[i];
            current_instruction[0] |= val;
          } else {
            current_instruction[0] |= 0x1f<<shift[i];
            current_instruction.push(to_int(p.value));
          }
          break;
        case 'NUMBER_LOOKUP':
        ...
        case 'LABEL_REF':
          ...
          if(typeof(address) == 'undefined')
            unresolved_labels.push({name:p.value,address:byte_code.length+2});
          current_instruction[0] |= 0x1f<<shift[i];
          current_instruction.push(to_int(address));
          break;
        case 'LABEL_REF_LOOKUP':
          var address = labels[p.value];
          if(typeof(address) == 'undefined')
            unresolved_labels.push({name:p.value,address:byte_code.length+1});
          current_instruction[0] |= 0x1e<<shift[i];
          current_instruction.push(to_int(address));
          break;
      }
